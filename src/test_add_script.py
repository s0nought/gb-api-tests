...
from modules.constants import BASE_URL_API, USER_NAME, ADD_SCRIPT_URL
...
def test_fn(api_session):
    form_html = (send_get_request(api_session, ADD_SCRIPT_URL)).text
    form_data = get_formdata(form_html, SCRIPT_DATA)
    res_html = (send_post_request(api_session, ADD_SCRIPT_URL, form_data)).text
